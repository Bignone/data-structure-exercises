...
pViaje Vehiculo::randomViaje(int dia)
{
    string letraEntradaAnterior = entrada.substr(4, 5);
    randomEntrada();
    string letraEntradaNueva = entrada.substr(4, 5);
    bool sonLetrasIguales = letraEntradaAnterior == letraEntradaNueva;

    while (sonLetrasIguales) {
        letraEntradaAnterior = entrada.substr(4, 5);
        randomEntrada();
        letraEntradaNueva = entrada.substr(4, 5);
        sonLetrasIguales = letraEntradaAnterior == letraEntradaNueva;
    }
    pViaje viaje = new Viaje(matricula, entrada);
    viaje->setDia(dia);
    viajes->insertarNodo(viaje);

    return viaje;

}

bool Vehiculo::buscarViaje(pViaje viaje)
{
    viajes->buscarViaje(viaje);
}

void Vehiculo::borrarViaje(pViaje viaje)
{
    viajes->borrarViaje(viaje);
}

void Vehiculo::insertarViaje(pViaje viaje)
{
    viajes->insertarNodo(viaje);
}
...
bool Vehiculo::pEquals(pVehiculo v)
{
    if (v==NULL) {
        return false;
    }
...
void Vehiculo::randomEntrada()
{
    ...
    random_entrada[random_entrada.size()-1] = taquillas[rand() % (sizeof(taquillas) - 1)];
...
int Vehiculo::pCompare(pVehiculo v)
{
    int result = 0;
    string letrasPropio = matricula.substr(4, 7);
    string letrasOtro = v->getMatricula().substr(4, 7);
...
void test_Vehiculo()
{
    ...
    cout << "Vehiculo: pVehiculos iguales (true):" << pVehiculoManual1->pEquals(pVehiculoManual1) << endl;
    cout << "Vehiculo: pVehiculos iguales (false):" << pVehiculoManual1->pEquals(pVehiculoManual2) << endl;
    cout << "Vehiculo: pVehiculos compare (-1):" << pVehiculoManual1->pCompare(pVehiculoManual2) << endl;
    cout << "Vehiculo: pVehiculos compare (+1):" << pVehiculoManual2->pCompare(pVehiculoManual1) << endl;
    cout << "Vehiculo: pVehiculos compare (0):" << pVehiculoManual2->pCompare(pVehiculoManual2) << endl;
    ...
    pViaje punteroViaje = new Viaje("asdasd", "asdasd");
    punteroVehiculo->randomViaje(3);
    punteroVehiculo->randomViaje(4);
    punteroVehiculo->insertarViaje(punteroViaje);
    punteroVehiculo->write();
    punteroVehiculo->borrarViaje(punteroVehiculo->getViajes()->getPrimero()->getViaje());
    punteroVehiculo->write();
